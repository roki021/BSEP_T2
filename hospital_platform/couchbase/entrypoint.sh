...
if [[ ${CLUSTER_HOST} ]];then
  ...
  curl --silent "http://${HOSTNAME}:8091/pools/default" \
    ...
    -d eventingMemoryQuota=${EVENTING_MEMORY_QUOTA}
  ...
  numbered_echo "Setting up bucket messages"
  curl -v -X POST "http://${HOSTNAME}:8091/pools/default/buckets" \
    -u ${USERNAME}:${PASSWORD} \
    -d name=messages \
    -d bucketType=couchbase \
    -d ramQuotaMB=${MEMORY_QUOTA}

  numbered_echo "Setting up bucket index memory"
  curl -v -X POST "http://${HOSTNAME}:8091/settings/indexes" \
    -u ${USERNAME}:${PASSWORD} \
    -d indexerThreads=4 \
